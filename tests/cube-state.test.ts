...
function createOll(): CubeStateOll {
  return createOll2(validBaseOllInput);
}

function createOll2(ollInput: string): CubeStateOll {
  return new CubeStateBuilder(ollInput, colors).buildOll(settings);
}

function createPll(): CubeStatePll {
  return createPll2(validBasePllInput);
}

function createPll2(pllInput: string): CubeStatePll {
  return new CubeStateBuilder(pllInput, colors).buildPll();
}
...
//  });
  ...
  it('should pre-set rotation values', () => {
    let co: CubeStateOll = createOll(); // new CubeStateOll(StringUtils.codeBlockToStrings(validBaseOllInput));
    let cp: CubeStatePll = createPll(); // new CubeStatePll(StringUtils.codeBlockToStrings(validBasePllInput));
...
describe('CubeState arrow coordinates', () => {
  ...
  it('result in empty non-null collections when no arrows given (OLL+PLL)', () => {
     ...
     * OLL
     */
    let co: CubeStateOll = createOll2(ollInputWithoutArrows);

    expect(co.algorithmToArrows).toBeTruthy();
    expect(co.algorithmToArrows.size()).toBe(0);
    expect(co.selectedAlgorithmHash).toBe('');
     ...
     * PLL
     */
    let cp: CubeStatePll = createPll2(pllInputWithoutArrows);
  ...
  it('result in static arrow array (PLL)', () => {
    let cp: CubeStatePll = createPll2(pllInputWithArrows);
  ...
  it('result in changeable arrow map (OLL)', () => {
    let co: CubeStateOll = createOll2(ollInputWithArrows);
