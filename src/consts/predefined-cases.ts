...
const toRecord = <T extends PredefinedCase>(prefix: string, cases: T[]): Record<string, T> => {
  const record: Record<string, T> = {}
  for (const item of cases) {
    record[`${prefix}-${item.id}`] = item
  }
  return record
}

// const collectIDs = (rec: Record<string, PredefinedCase>): string[] => Object.keys(rec)
...
const createPredefinedCaseRubikOll = (input: {
  id: number,
  desc: string,
  ollPattern: OllPattern,
  algorithms: string[]
}): PredefinedCaseRubikOll => {
  return {
    id: input.id,
    desc: input.desc,
...
const createPredefinedCaseRubikPll = (input: {
  id: number,
  desc: string,
  arrows: string,
  algorithms: string[]
}): PredefinedCaseRubikPll => {
  return {
    id: input.id,
    desc: input.desc,
